...
@app.route("/verify", methods=["GET", "POST"])
def verify():
    phone_number_with_country_code =  "+91" + str(current_user.phone_number)
    account_sid = os.environ['TWILIO_ACCOUNT_SID']
    auth_token = os.environ['TWILIO_AUTH_TOKEN']
    
    client = Client(account_sid, auth_token)
    
    service = client.verify.v2.services.create(friendly_name='Devops_Twilio')
    print(service.sid)
    
    verification = client.verify.v2.services(service.sid).verifications.create(to=phone_number_with_country_code, channel="sms")
    if request.method == "POST":
